...
class ReleaseNotesGenerator:
    def __init__(self, args):
        ...
        self.start_date, self.end_date = self._detect_time_period_from_last_run()
    ...
    def get_tags_in_period(self, repo, start_date, end_date):
        ...
        try:
            ...
            for tag in recent_tags:
                try:
                    commit_date = self._fix_tz(tag.commit.commit.author.date)

                    # Check if date is in our range
                    if self._fix_tz(start_date) <= commit_date <= self._fix_tz(end_date):
                        tags.append({
                            'name': tag.name,
                            'date': commit_date,
            ...
            logger.info(f"Found {len(tags)} tags in {repo.name} within date range {start_date.strftime('%Y-%m-%d')} "
    ...
    def generate_timeline(self):
        ...
        for repo in repos:
            ...
            tags = self.get_tags_in_period(repo, self.start_date, self.end_date)
            ...
            for tag in tags_to_process:
                ...
                try:
                    # Get tags before this tag
                    tag_date = tag['date']
                    previous_tag = None
                    
                    # Directly get tags and convert to a list immediately to avoid PaginatedList issues
                    all_tags = []
                    # Limit to 50 recent tags for efficiency
                    ...
                    # Sort tags by date
                    sorted_tags = sorted(all_tags, key=lambda t: self._fix_tz(t.commit.commit.author.date))
                    
                    # Find the most recent tag that's older than our current tag
                    for t in sorted_tags:
                        t_date = self._fix_tz(t.commit.commit.author.date)
                    ...
                    if not previous_tag:
                        ...
                        if commits:
                            ...
                            previous_tag = {
                                ...
                                'date': self._fix_tz(initial_commit.commit.author.date),
...
def main():
    ...
    parser.add_argument('--ai-summary', action='store_true',
                        help='Generate AI summary of changes (requires OPENAI_API_KEY)')
