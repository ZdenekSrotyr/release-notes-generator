...
from src.github_utils import initialize_github_client, get_repositories, get_tags_in_period, get_changes_between_tags, fix_timezone
...
class ReleaseNotesGenerator:
    ...
    def __init__(self, github_token, start_date=None, end_date=None, organization="keboola"):
        ...
        if start_date and end_date:
            ...
            logger.info(f"Using specified date range: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        else:
            # Detect from last run or use default of 30 days
            self.start_date, self.end_date = detect_time_period_from_last_run(days=30)
            logger.info(f"Using date range: {self.start_date.strftime('%Y-%m-%d')} to {self.end_date.strftime('%Y-%m-%d')}")
