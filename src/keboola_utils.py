...
def save_release_to_table(ci: CommonInterface, release_data: Dict[str, Any], table_name: str = "releases") -> bool:
    ...
    logger.info(f"Starting save_release_to_table for {release_data.get('component_name', 'unknown')} {release_data.get('tag_name', 'unknown')}")
    try:
        # Generate release note content
        logger.info(f"Generating release note content for {release_data.get('component_name', 'unknown')} {release_data.get('tag_name', 'unknown')}")
        try:
        ...
        table_data = {
            'release_date': release_data['date'].isoformat() if hasattr(release_data['date'], 'isoformat') else str(release_data['date']),
            ...
            'changes_list': json.dumps([change.get('title', '') for change in release_data.get('changes', [])], ensure_ascii=False, default=str) if release_data.get('changes') else '[]',  # Added .get() with default empty list
                ...
                with open(csv_path, 'r', encoding='utf-8') as csvfile:
                    ...
                    for row in reader:
                        ...
                        if (row.get('component_id') == table_data['component_id'] and 
                            ...
                            is_duplicate = True
                            break
                    logger.info(f"Checked {sum(1 for _ in csv.DictReader(open(csv_path, 'r', encoding='utf-8')))} existing rows for duplicates")
            except Exception as e:
        ...
        if not is_duplicate:
            ...
            logger.info(f"Successfully saved release {release_data['component_name']} {release_data['tag_name']} to table with content (component_id: {release_data['component_name']})")
            return True
        else:
